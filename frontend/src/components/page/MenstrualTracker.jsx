...
// Click "end of period" => send start and end dates to backend
  const handleEndPeriod = async () => {
    ...
    try {
      ...
      alert("Failed to save, please check the backend service or network connection");
    }
  };
  const handleDeletePeriod = async (recordId) => {
    ...
    const confirmDelete = window.confirm("Are you sure you want to delete this menstrual period record?");
...
console.log("📊 chartData:", chartData);
      ...
      <div className="tracker-container">
        ...
        <div className="calendar-section">
            ...
            tileClassName={({ date, view }) => {
              if (view === "month") {
                let matchedDay = calendarData.find(
                  (entry) => moment(entry.date).format("YYYY-MM-DD") === moment(date).format("YYYY-MM-DD")
                );
                if (matchedDay) {
                  if (matchedDay.type === "past") return "past-period";
                  if (matchedDay.type === "predicted") return "predicted-period";
                  if (matchedDay.type === "ovulation") return "ovulation-period";
                }
            }
            return null;
            }}
          ...
          <div className="period-buttons">
            <button className="period-btn start" onClick={handleStartPeriod} > <img src={cute} alt="cute" className="cute-icon" />Start time</button>
            <button  className="period-btn end" onClick={handleEndPeriod}> <img src={cute} alt="cute" className="cute-icon" />End time</button>
          </div>
        </div>
  ...
  <span className="days">{nextPeriodDays !== null ? nextPeriodDays : "--"}</span>
  ...
  <div className="record-info">
    <span className="record-dates">
      <strong>{new Date(period.startDate).toLocaleDateString()}</strong>
      {" - "}
      {new Date(period.endDate).toLocaleDateString()} (
      {period.duration || "?"} days)
    </span>
  </div>
  <button className="delete-btn" onClick={() => handleDeletePeriod(period.id)}>❌</button>
</div>
